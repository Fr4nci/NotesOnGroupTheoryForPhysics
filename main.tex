...
\begin{document}
	...
	Riporto all'inizio del libro le notazioni adottate all'interno di questo documento (se non specificate dal contesto):
 	\begin{itemize}
		...
		\item $S^1$ circonferenza unitaria
		\item $S^2$ sfera unitaria
		\item $S^{n+1}$ sfera $n$ dimensionale
		\item $\mathbb{R}$ campo dei numeri reali
		\item $\mathbb{C}$ campo dei numeri complessi
		\item $\mathbb{N}$ insieme dei numeri naturali
		\item $\mathbb{Z}$ insieme dei numeri relativi
		\item $\mathbb{Z}_n$ insieme dei numeri relativi $\text{mod} \, n$
		\item $\mathbb{Q}$ insieme dei numeri razionali
		\item $S^i$ sfera unitaria immersa in $i+1$ dimensioni
		\item $D^i$ palla unitaria immersa in $i$ dimensioni
	\end{itemize}
	\newpage
	\include{capitoli/intro.tex}
	\include{capitoli/struttura_gruppo}
	\newpage
	\pagebreak
	\newpage
	\clearpage
