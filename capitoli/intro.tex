...
\section{Classificazione dei gruppi}
...
\begin{proof}
    ...
    Infine, la mappa conserva l'operazione di gruppo:
    \begin{align}
        \bar{f}(aK \circ_1 bK) \stackrel{\text{associat.}}{=}& \bar{f}(a \circ_1 (Kb) \, \circ_1 K) \stackrel{\text{lemma }\ref{lemma:eq_coset_ker}}{=} \bar{f}(a \, \circ_1 \, (bK) \, \circ_1 K) = \bar{f}(abK) = f(ab) = \nonumber \\
		&= f(a)f(b) = \bar{f}(aK)\bar{f}(bK).
    \end{align}
